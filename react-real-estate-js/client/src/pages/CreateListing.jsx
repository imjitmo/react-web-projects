...
export default function CreateListing() {
  ...
  const handleImageSubmit = () => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      Promise.all(promises)
        ...
        .catch((err) => {
          setImageUploadError('Image upload failed! Image size must be below 2mb!');
          setUploading(false);
          err;
        });
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire({
        icon: 'success',
        title: 'Created Listing Successful!',
        text: data.message,
  ...
  return (
    <main className="p-3 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-column gap-4 sm:flex-row">
        ...
        <div className="flex flex-col gap-4 flex-1">
          <p className="font-semibold">
            Images:
            <span className="font-normal text-sm text-gray-700 ms-1">
            ...
            formData.imageUrls.map((url, i) => (
              <div key={url} className="flex justify-between p-3 border items-center">
                ...
                <button
                  ...
                  className="text-red-500 p-3 font-semibold rounded-lg uppercase hover: opacity-90"
                >
                  delete
                </button>
              </div>
          ...
          <button
            type="submit"
            className="p-3 bg-orange-400 text-white rounded-lg uppercase hover:bg-slate-700 disabled:opacity-70"
