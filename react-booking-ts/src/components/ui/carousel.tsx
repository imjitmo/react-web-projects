...
function CarouselDots({ className, ...props }: React.ComponentProps<'div'>) {
  ...
  return (
    <div
      role="tablist"
      className={cn('absolute top-2/3 w-full flex items-center justify-center gap-2', className)}
      {...props}
    >
      {api?.scrollSnapList().map((_, index) => (
        <button
          ...
          className={cn(
            'size-4 rounded-full border-2  cursor-pointer border-slate-50',
            index === selectedIndex ? 'bg-yellow-400' : 'bg-transparent'
          )}
